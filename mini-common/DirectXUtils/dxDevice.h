...
namespace mini
{
	...
	public:
		...
		{
			auto desc = utils::BufferDescription::VertexBufferDescription(N * sizeof(T));
		...
		{
			return CreateBuffer(
				utils::BufferDescription::VertexBufferDescription(static_cast<UINT>(sizeof(T)*count)),
		...
		dx_ptr<ID3D11GeometryShader> CreateGeomatryShader(const std::wstring& filename) const
		{
